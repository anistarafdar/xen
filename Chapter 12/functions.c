...
lval* lval_call(lenv* e, lval* f, lval* a) {
	...
	// while arg still remain to be processed
	while (a->count) {
		// if we ran out of formal arguments to bind
		if (f->formals->count == 0) {
			lval_del(a); return lval_err(
				"Functions passed too many arguments. "
				"Got %i, Expected %i", given, total);
		}
		//pop the furst symbol from the formals
	...
	if (f->formals->count == 0) {
		...
		// otherwise return partiially evaluated function
		return lval_copy(f);
	}
}
